...
## Summary Inferential Statistics for Full Demographic Model

```{r}
full.model = glm(event ~ stag + gender + age + industry + profession + traffic + coach + head_gender + greywage + way , 
...
---
## Stepwise AIC Algorithm for Stepwise Demographic Model
...
---
## Summary Inferential Statistics for Stepwise Demographic Model
...
---
## Goodness of Fit for two Demographic Models
...
AIC - Akaike information criterion (AIC), a single number score estimates models relatively to other models
<br>
```{r}
...
<br>
Full model: χ2(24.788, 10) = 0.005762
Stepwise model: χ2(19.313, 5) = 0.001680

Stepwise model chosen as final model due to smaller p-value and AIC values

---
## Summary statistics of final demographic model and Odds Ratios
...
---
## Model Diagnostics for Demographic Model
...
goodness=rbind(full.model.big5 = global.measure(full.model.big5),
      stepwise.mode.big5 =global.measure(final.model.big5))
row.names(goodness) = c("full.model.big5", "stepwise.mode.big5")
kable(goodness, caption ="Comparison of global goodness-of-fit statistics")

```
<br>
Full model: χ2(7.71, 5) =  0.172959
Stepwise model: χ2(7.04, 2) = 0.029599
...
While these models are an improvement over using a the intercept, the improvement is not enough to be meaningfully accurate. When tested with a bootstrapped 5-fold CV both demographic model and personality trait model returned accuracies of roughly 54%
